import type { Core } from '@strapi/strapi';
import { ROLE_NAMES } from '../utils/roles';

type RoleDef = { name: string; type: string; description: string };

const LMS_ROLES: RoleDef[] = [
  { name: ROLE_NAMES.ADMIN, type: 'lms-admin', description: 'LMS administrator' },
  {
    name: ROLE_NAMES.CONTENT_MANAGER,
    type: 'content-manager',
    description: 'Manages content and blog',
  },
  { name: ROLE_NAMES.INSTRUCTOR, type: 'instructor', description: 'Creates and teaches courses' },
  { name: ROLE_NAMES.STUDENT, type: 'student', description: 'Enrolls and learns' },
];

/** Student-only learning actions — must not leak to staff roles (permission matrix). */
const STUDENT_ONLY_ACTIONS = [
  'api::lms.lms.enroll',
  'api::lms.lms.myCourses',
  'api::lms.lms.completeLesson',
  'api::lms.lms.takeQuiz',
  'api::lms.lms.submitQuiz',
  'api::lms.lms.studentDashboard',
  'api::lms.lms.listMyAssignments',
  'api::lms.lms.submitAssignment',
];

/** Shared profile / catalog / read for all authenticated LMS roles */
const SHARED_AUTH_ACTIONS = [
  'api::lms.lms.me',
  'api::lms.lms.updateMe',
  'api::lms.lms.changeMyPassword',
  'api::lms.lms.courseProgress',
  'api::lms.lms.quizAttempts',
  'api::lms.lms.myCertificates',
  'api::lms.lms.getCertificate',
  'api::lms.lms.getCoursePlayer',
  'api::lms.lms.listCatalog',
  'api::lms.lms.getCatalogCourse',
  'api::lms.lms.listCategories',
  'api::lms.lms.listBlog',
  'api::lms.lms.getBlogBySlug',
  'api::lms.lms.listBanners',
  'api::lms.lms.listAnnouncements',
  'api::lms.lms.listMyNotifications',
  'api::lms.lms.markNotificationRead',
  'api::lms.lms.verifyCertificate',
  'plugin::users-permissions.auth.changePassword',
  'plugin::users-permissions.user.me',
];

const COURSE_AUTHORING_ACTIONS = [
  'api::lms.lms.listCourseModules',
  'api::lms.lms.createCourseModule',
  'api::lms.lms.updateCourseModule',
  'api::lms.lms.deleteCourseModule',
  'api::lms.lms.createCourse',
  'api::lms.lms.updateCourse',
  'api::lms.lms.deleteCourse',
  'api::lms.lms.createLesson',
  'api::lms.lms.updateLesson',
  'api::lms.lms.deleteLesson',
  'api::lms.lms.createQuiz',
  'api::lms.lms.updateQuiz',
  'api::lms.lms.deleteQuiz',
  'api::lms.lms.staffListCourses',
  'api::lms.lms.staffListProgress',
  'api::lms.lms.instructorDashboard',
  'api::lms.lms.staffListAssignments',
  'api::lms.lms.staffCreateAssignment',
  'api::lms.lms.staffListSubmissions',
  'api::lms.lms.staffGradeSubmission',
];

const CONTENT_MANAGER_ACTIONS = [
  ...SHARED_AUTH_ACTIONS,
  ...COURSE_AUTHORING_ACTIONS,
  'api::lms.lms.contentManagerDashboard',
  'api::lms.lms.adminListBanners',
  'api::lms.lms.adminCreateBanner',
  'api::lms.lms.adminUpdateBanner',
  'api::lms.lms.adminDeleteBanner',
  'api::lms.lms.adminListCategories',
  'api::lms.lms.adminCreateCategory',
  'api::lms.lms.adminUpdateCategory',
  'api::lms.lms.adminDeleteCategory',
  'api::lms.lms.manageBlog',
  'api::lms.lms.createBlog',
  'api::lms.lms.updateBlog',
  'api::lms.lms.deleteBlog',
];

const INSTRUCTOR_ACTIONS = [
  ...SHARED_AUTH_ACTIONS,
  ...COURSE_AUTHORING_ACTIONS,
];

const STUDENT_ACTIONS = [
  ...SHARED_AUTH_ACTIONS,
  ...STUDENT_ONLY_ACTIONS,
  'api::lms.lms.validateCoupon',
  'api::lms.lms.createOrderCheckout',
  'api::lms.lms.payOrder',
];

const ADMIN_ACTIONS = [
  ...SHARED_AUTH_ACTIONS,
  ...COURSE_AUTHORING_ACTIONS,
  'api::lms.lms.contentManagerDashboard',
  'api::lms.lms.adminDashboard',
  'api::lms.lms.adminListUsers',
  'api::lms.lms.adminUpdateUserRole',
  'api::lms.lms.adminUpdateUserStatus',
  'api::lms.lms.adminCreateUser',
  'api::lms.lms.adminDeleteUser',
  'api::lms.lms.adminListCertificates',
  'api::lms.lms.adminListEnrollments',
  'api::lms.lms.adminListBanners',
  'api::lms.lms.adminCreateBanner',
  'api::lms.lms.adminUpdateBanner',
  'api::lms.lms.adminDeleteBanner',
  'api::lms.lms.adminListCoupons',
  'api::lms.lms.adminCreateCoupon',
  'api::lms.lms.adminUpdateCoupon',
  'api::lms.lms.adminDeleteCoupon',
  'api::lms.lms.validateCoupon',
  'api::lms.lms.adminListCategories',
  'api::lms.lms.adminCreateCategory',
  'api::lms.lms.adminUpdateCategory',
  'api::lms.lms.adminDeleteCategory',
  'api::lms.lms.manageBlog',
  'api::lms.lms.createBlog',
  'api::lms.lms.updateBlog',
  'api::lms.lms.deleteBlog',
  'api::lms.lms.adminListByRole',
  'api::lms.lms.adminForceEnroll',
  'api::lms.lms.adminRemoveEnrollment',
  'api::lms.lms.adminCrudList',
  'api::lms.lms.adminCrudCreate',
  'api::lms.lms.adminCrudUpdate',
  'api::lms.lms.adminCrudDelete',
  'api::lms.lms.adminStockAdjust',
  'api::lms.lms.adminRevokeCertificate',
  'api::lms.lms.createOrderCheckout',
  'api::lms.lms.payOrder',
  'api::lms.lms.adminGlobalSearch',
  'api::lms.lms.adminReportsSummary',
  'api::lms.lms.adminGetSettings',
  'api::lms.lms.adminSaveSettings',
  'plugin::users-permissions.auth.callback',
  'plugin::users-permissions.auth.connect',
  'plugin::users-permissions.auth.emailConfirmation',
  'plugin::users-permissions.auth.forgotPassword',
  'plugin::users-permissions.auth.resetPassword',
];

/** Actions enabled per role for custom LMS API + auth */
const ROLE_ACTIONS: Record<string, string[]> = {
  [ROLE_NAMES.ADMIN]: ADMIN_ACTIONS,
  [ROLE_NAMES.CONTENT_MANAGER]: CONTENT_MANAGER_ACTIONS,
  [ROLE_NAMES.INSTRUCTOR]: INSTRUCTOR_ACTIONS,
  [ROLE_NAMES.STUDENT]: STUDENT_ACTIONS,
};

const PUBLIC_ACTIONS = [
  'api::lms.lms.listCatalog',
  'api::lms.lms.getCatalogCourse',
  'api::lms.lms.listBlog',
  'api::lms.lms.getBlogBySlug',
  'api::lms.lms.listBanners',
  'api::lms.lms.listCategories',
  'api::lms.lms.verifyCertificate',
  'api::lms.lms.listAnnouncements',
  'plugin::users-permissions.auth.callback',
  'plugin::users-permissions.auth.connect',
  'plugin::users-permissions.auth.emailConfirmation',
  'plugin::users-permissions.auth.forgotPassword',
  'plugin::users-permissions.auth.resetPassword',
  'plugin::users-permissions.auth.register',
  'plugin::users-permissions.auth.sendEmailConfirmation',
];

async function ensurePermission(strapi: Core.Strapi, roleId: number, action: string) {
  const existing = await strapi.db.query('plugin::users-permissions.permission').findOne({
    where: { action, role: roleId },
  });
  if (!existing) {
    await strapi.db.query('plugin::users-permissions.permission').create({
      data: { action, role: roleId },
    });
  }
}

async function revokePermission(strapi: Core.Strapi, roleId: number, action: string) {
  const existing = await strapi.db.query('plugin::users-permissions.permission').findOne({
    where: { action, role: roleId },
  });
  if (existing) {
    await strapi.db.query('plugin::users-permissions.permission').delete({
      where: { id: existing.id },
    });
  }
}

export async function ensureLmsRoles(strapi: Core.Strapi) {
  const roleMap: Record<string, any> = {};

  for (const def of LMS_ROLES) {
    let role = await strapi.db.query('plugin::users-permissions.role').findOne({
      where: { name: def.name },
    });
    if (!role) {
      role = await strapi.db.query('plugin::users-permissions.role').create({
        data: {
          name: def.name,
          description: def.description,
          type: def.type,
        },
      });
      strapi.log.info(`[LMS] Created role: ${def.name}`);
    }
    roleMap[def.name] = role;
  }

  // Public role permissions
  const publicRole = await strapi.db.query('plugin::users-permissions.role').findOne({
    where: { type: 'public' },
  });
  if (publicRole) {
    for (const action of PUBLIC_ACTIONS) {
      await ensurePermission(strapi, publicRole.id, action);
    }
  }

  // Also keep default authenticated role usable for me/auth
  const authenticated = await strapi.db.query('plugin::users-permissions.role').findOne({
    where: { type: 'authenticated' },
  });
  if (authenticated) {
    for (const action of ROLE_ACTIONS[ROLE_NAMES.STUDENT]) {
      await ensurePermission(strapi, authenticated.id, action);
    }
  }

  for (const [roleName, actions] of Object.entries(ROLE_ACTIONS)) {
    const role = roleMap[roleName];
    if (!role) continue;
    for (const action of actions) {
      await ensurePermission(strapi, role.id, action);
    }
  }

  // Harden matrix: strip student-only learning actions from staff roles
  for (const staffRole of [
    ROLE_NAMES.ADMIN,
    ROLE_NAMES.CONTENT_MANAGER,
    ROLE_NAMES.INSTRUCTOR,
  ]) {
    const role = roleMap[staffRole];
    if (!role) continue;
    for (const action of STUDENT_ONLY_ACTIONS) {
      await revokePermission(strapi, role.id, action);
    }
  }

  return roleMap;
}
